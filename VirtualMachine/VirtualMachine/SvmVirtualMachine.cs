...
public sealed class SvmVirtualMachine: IVirtualMachine
{
    ...
    private List<int> debugLines = new List<int>();
    StringDictionary location = new StringDictionary(); 
    #endregion
    ...
    public SvmVirtualMachine(string filepath)
    {
        ...
        try
        {
            if (JITCompiler.loadedAssemblies.Count() != 0)
            {
                foreach (Assembly asm in JITCompiler.loadedAssemblies)
                {
                    if (asm != null)
                    {
                        foreach (TypeInfo type in asm.DefinedTypes)
                        {
                            if (type.ImplementedInterfaces.Contains(typeof(IDebugger)))
                            {
            ...
            {
                JITCompiler.loadAssembliesWithInstruction();
                foreach (Assembly asm in JITCompiler.loadedAssemblies)
                {
                    foreach (TypeInfo type in asm.DefinedTypes)
                    {
        ...
        {
            if (ex is SvmRuntimeException)
            {
                Console.WriteLine(RuntimeErrorMessage, ex.Message);
            }
            return;
        }
    ...
    public int ProgramCounter
    {
        ...
        public StringDictionary Location
    {
        get
        {
            return location;
    ...
    {
        var item = program[programCounter];
        var itemIndex = program.IndexOf(item);

        var isExist = debugLines.Contains(itemIndex);
        if (isExist && debugger != null)
        {
            ...
            try
            {
                    foreach (Assembly asm in JITCompiler.loadedAssemblies)
                    {
                        if (asm != null)
                        {
                            foreach (TypeInfo type in asm.DefinedTypes)
                            {
                                if (type.ImplementedInterfaces.Contains(typeof(IDebugFrame)))
                                {
                                    debugFrame = (IDebugFrame)Activator.CreateInstance(type);
                                    break;
                                }
                            }
                        }
            ...
            debugFrame.CurrentInstruction = item;
    ...
    private void ParseInstruction(string instruction, int lineNumber)
    {
        ...
        if (tokens[0] == "*")
        {
            debugLines.Add(lineNumber);
            tokens = tokens.Skip(1).ToArray();
        }
        
        if (tokens[0].Contains('%'))
        {
            var label = tokens[0].Skip(1).ToArray();
            var label2 = label.SkipLast(1).ToArray();
            string label3 = new string(label2);
            location.Add(label3, lineNumber.ToString());
            tokens = tokens.Skip(1).ToArray();
        }
        #endregion
